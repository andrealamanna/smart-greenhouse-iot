...
RESOURCE(res_lamp,
         "title=\"Lamp Actuator: ?PUT state=ON|OFF&zone=<value>\";if=\"actuator\";rt=\"Control\"",
         res_get_handler,
...
static void res_post_put_handler(coap_message_t *request, coap_message_t *response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset)
{
  ...
  if(len > 0) {
    ...
    if(strncmp(state, "ON", len) == 0) {
      lamp_state = 1;
      LOG_DBG("Lamp Activated! \n");
      leds_set(LEDS_NUM_TO_MASK(LEDS_RED));
    } 
    else if(strncmp(state, "OFF", len) == 0) {
      lamp_state = 0;
      LOG_DBG("Lamp Stopped \n");
...
static void res_get_handler(coap_message_t *request, coap_message_t *response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset)
{
  ...
  snprintf((char *)buffer, COAP_MAX_CHUNK_SIZE, "{\"state\":%d}", lamp_state);
